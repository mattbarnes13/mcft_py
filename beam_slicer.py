...
def cross_sec_slice(beam_type, beam_dimensions, trans_reo,  num_slices=20):
    ...
    OUTPUTS:
    slice_list - list of length n containing [b1, h_i, rho_syi, y_ci] for each slice
        where
        b1 - is the spalled width (mm) of the slice,
        h_i - is the thickness or height of the slice (mm)
        rho_syi -is the ratio of transverse reinforcement (mm)
        y_ci - is the distance from slice centroid to the top of the section

    I -  the second moment of intertia (mm^4)
    y_ci - the height of centroid from bottom of the section (mm)
    """
    # beam type
